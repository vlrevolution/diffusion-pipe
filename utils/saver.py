...
def need_to_checkpoint(config, epoch=None):
    ...
    if epoch is not None:
        if (
            "checkpoint_every_n_epochs" in config
    ...
    if is_main_process():
        ...
        elif (current_time - last_checkpoint_time) / 60 > config[
...
class Saver:
    ...
    def save_adapter(self, name):
        ...
        # Only the master process on the first stage saves the model.
        if dp_id == 0 and stage_id == 0:
            os.makedirs(save_dir, exist_ok=True)

            # The adapter_model (PEFT or LyCORIS) lives on the model object.
            # Both libraries provide a state_dict() method that returns only the adapter weights.
            state_dict = self.model.adapter_model.state_dict()
        ...
        # Barrier to ensure all processes wait for the save to complete.
        dist.barrier()
    ...
    def process_step(self, step, examples):
        ...
        if (
            "save_every_n_steps" in self.config
